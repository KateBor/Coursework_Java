...
public class Warehouse2ServiceImpl implements Warehouse2Service{
    ...
    Optional<Warehouse2> optionalWarehouse2 = warehouse2Repository.findById(id);
        if (optionalWarehouse2.isPresent()) {
            return optionalWarehouse2.get();
        } else {
    ...
    @Transactional
    @Override
    public void addGoodsToWarehouse2(WarehouseCreationRequest warehouse) {
        ...
        if (optionalGood.isPresent()) {
            ...
            if (warehouse2 != null) {
                warehouse2Repository.updateGood_count(warehouse2.getId(), warehouse2ToCreate.getGood_count());
                warehouse2Repository.findByGood_id(warehouse.getGood_id());
            } else {
    ...
    public void updateGoodsCount(Long good_id, int count) {
        Warehouse2 opWarehouse2 = warehouse2Repository.findByGood_id(good_id);
        if (opWarehouse2 != null) {
            warehouse2Repository.updateGood_count(good_id, count);
        } else {
            Optional<Good> optionalGood = goodsRepository.findById(good_id);
            if (optionalGood.isPresent()) {
                Warehouse2 warehouse2 = new Warehouse2();
                warehouse2.setGood(optionalGood.get());
                warehouse2.setGood_count(count);
